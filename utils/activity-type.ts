...
const typeMatchers: Record<ActivityType, (type: string) => boolean> = {
  ...
  other: (type: string) => !type.includes('ride') && !type.includes('run') && !type.includes('walk')
}

export function getActivityType(activity: StravaActivity): ActivityType {
  const sportType = activity.sport_type.toLowerCase()
  return Object.entries(typeMatchers).find(([_, matcher]) => 
    matcher(sportType)
  )?.[0] as ActivityType || 'other'
}

export function filterActivitiesByType(activities: StravaActivity[], type: ActivityType): StravaActivity[] {
